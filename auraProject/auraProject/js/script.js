...
function spawnFlyingStar(fromBtn){
  ...
  const dayNames=["Mon","Tue","Wed","Thu","Fri","Sat","Sun"];
    ...
    function render() {
        ...
        const stores = [...new Map(baseTasks.map(t => [t.store_id, {id: t.store_id, name: t.do_staff_store_name}])).values()];
        dom.storeFilter.innerHTML = `<option value="all">Tất cả cửa hàng</option>` + stores.map(s => `<option value="${s.id}" ${state.storeFilter == s.id ? 'selected' : ''}>${s.name}</option>`).join('');
        ...
        // 3. Lấy danh sách nhân viên tương ứng và render
        const staffs = [...new Map(baseTasks.map(t => [t.do_staff_id, {id: t.do_staff_id, name: t.do_staff_name}])).values()];
        dom.staffFilter.innerHTML = `<option value="all">Tất cả nhân viên</option>` + staffs.map(s => `<option value="${s.id}" ${state.staffFilter == s.id ? 'selected' : ''}>${s.name}</option>`).join('');
        ...
        const tasksToDisplay = (!state.statusFilter || state.statusFilter === 'all')
            ? baseTasks // baseTasks now is already filtered by store/staff
    ...
    function showTaskPopup(taskId) {
        ...
        const responseTypeName = state.codeMap[task.response_type_id] || 'Unknown';
    
        let responseContent = '';
        // Logic để render nội dung tương tác dựa trên response_type
        if (isInteractive) {
                ...
                case 'Yes-No':
                    responseContent = `
                        <div class="response-yes-no">
                            <label><input type="radio" name="yes-no-response" value="Yes"> Yes</label>
                            <label><input type="radio" name="yes-no-response" value="No"> No</label>
                        </div>
            ...
            // Chế độ chỉ xem cho Done/Overdue
            switch (responseTypeName) {
                case 'Check list':
                    const checks = task.check_lists || [];
                    let tableHTML = `<table><thead><tr><th>STT</th><th>Check Name</th><th>Status</th><th>Time</th></tr></thead><tbody>`;
                    checks.forEach((c, i) => {
                        tableHTML += `<tr><td>${i + 1}</td><td>${c.check_list_name}</td><td>${c.check_status || ''}</td><td>${c.completed_at && c.check_status === "Done" ? formatDate(c.completed_at) : ''}</td></tr>`;
                    });
                    tableHTML += '</tbody></table>';
                    responseContent = tableHTML;
                    break;
                default:
                    responseContent = `<p><strong>Status:</strong> ${task.status_name}.</p>`;
            }
        }
    ...
    function setupEventListeners() {
        ...
        dom.popup.addEventListener('click', async (e) => {
            ...
            if (e.target.type === 'checkbox') {
                const taskId = Number(dom.popup.querySelector('h3').textContent.match(/#(\d+)/)[1]);
                ...
                // Re-fetch and re-render to get updated task status
                await initialize();
            }
        });
    }
