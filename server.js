...
app.post("/add-task", (req, res) => {
    ...
    fs.readFile(filePath, "utf8", (err, data) => {
        ...
        fs.writeFile(filePath, JSON.stringify(tasks, null, 2), (err) => {
            ...
            res.json({ message: "Lưu task to do thành công", task: newTask });
        });
    });
});

// API: thêm repeat task mới
app.post("/repeat-task", (req, res) => {
    const newTask = req.body;

    // Đường dẫn file JSON
    const filePath = __dirname + "/screen-create-task/repeat-task.json";
    ...
    fs.readFile(filePath, "utf8", (err, data) => {
        ...
        fs.writeFile(filePath, JSON.stringify(tasks, null, 2), (err) => {
            if (err) return res.status(500).json({ error: "Lỗi ghi file" });
            res.json({ message: "Lưu repeat task thành công", task: newTask });
        });
    });
});
